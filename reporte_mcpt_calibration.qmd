...
Cuando un modelo de *machine learning* enseña una curva de equity bonita en un backtest, casi siempre hay que preguntarse algo incómodo: ¿encontró una señal predictiva, o nada más memorizó ruido? El sobreajuste es la razón por la que tantos sistemas que se ven rentables en papel queman capital en cuanto se prenden en producción.

El fondo del asunto es viejo y conocido: con suficientes parámetros y suficientes intentos, cualquier modelo aprende los accidentes históricos del precio y produce métricas espectaculares dentro de la muestra. La pregunta útil no es "¿mi sistema gana dinero en backtest?", sino "¿gana significativamente más de lo que ganaría un sistema sin señal sobre datos sin información predictiva?". Esa es la pregunta que ataca el Monte Carlo Permutation Test (MCPT) que Timothy Masters describe en *Testing and Tuning Market Trading Systems*.

## Objetivo y pregunta de investigación

Este reporte no intenta construir el mejor sistema de trading sobre el S&P 500. Es un trabajo de calibración metodológica. La pregunta es una sola:
...
Para responderla armamos un experimento con tres modelos donde sabemos *a priori* qué debería pasar:
...
Si el MCPT está bien calibrado, esperamos p-values bajos para el oráculo (señal real), p-values bajos in-sample pero altos walk-forward para el LGBM (overfitting clásico), y p-values más o menos uniformes para el random.
...
1. Calcular el desempeño real del sistema con los datos en su orden original. Llamamos a ese valor $\theta_{\text{real}}$.
...
4. Repetir $m$ veces (centenas o miles).
...
El "+1" en numerador y denominador es la corrección de Masters: la corrida original cuenta como una observación válida bajo $H_0$. Si $\theta_{\text{real}}$ cae a la derecha de casi todas las permutaciones, $k$ es chico y el p-value también: hay evidencia de señal genuina.

## ¿Qué permutar y por qué? (la decisión más importante)

Aquí se gana o se pierde el experimento, y por algo Masters le dedica buena parte del capítulo. Hay dos estrategias grandes.

### Permutación de precios (Masters)

Sirve para sistemas de trading que miran la historia de precios y toman decisiones de long/short. Se deconstruye el precio en cambios (log-returns), se barajan los cambios y se reconstruye una historia sintética manteniendo el precio inicial y final (así se conserva la tendencia). El sistema se re-entrena sobre ese "mercado sintético".

### Permutación del target — *Y-permutation* (Masters)

Sirve para modelos predictivos donde uno tiene pares $(X_t, y_t)$ y quiere romper el link entre features y target sin tocar la estructura interna de $X$. Es nuestro caso: features técnicas sobre SPY para predecir el signo del retorno del día siguiente.

Nosotros usamos Y-permutation. La justificación que da Masters es:
...
En otras palabras: si tenemos como features las medias móviles de 10, 20 y 50 días, más la volatilidad realizada, todas esas series están correlacionadas entre sí en el tiempo real. Permutarlas por separado generaría combinaciones que en el mercado real no ocurren (volatilidad alta con todas las medias móviles planas, por ejemplo). El MCPT pide que las permutaciones tengan la misma probabilidad de ocurrir bajo $H_0$ que en la realidad, y permutar features de forma independiente rompe eso.

La opción limpia es dejar $X$ intacto y barajar sólo $y$ (junto con los retornos $r$ que se usan para evaluar PnL). Así rompemos el emparejamiento $X_t \to y_t$ sin contaminar la estructura interna de $X$.
...
Las features técnicas (medias móviles, volatilidades realizadas) son extremadamente autocorrelacionadas: el valor en $t$ y en $t-1$ son casi idénticos. Si el target o el retorno también lo son, el MCPT se rompe.

En nuestro caso el target es el signo del log-return diario, que tiene autocorrelación prácticamente nula (lo cual es consistente con la hipótesis de eficiencia débil). El problema más fino es otro: algunas features están correlacionadas con la magnitud del retorno futuro. Por ejemplo, la volatilidad realizada predice cuán grande será el próximo movimiento, aunque no en qué dirección va. Eso le da una ventaja sutil a la corrida real, porque el modelo "sabe cuándo" los retornos son grandes incluso si no sabe el signo.
...
La diferencia entre los dos p-values aísla el sesgo que inducen las features correlacionadas con la magnitud.
...
Corremos los dos extremos:

- **MCPT in-sample**: entrenamos el modelo sobre todos los datos y medimos el Profit Factor (PF) in-sample. Bajo $H_0$ permutamos $y$ y volvemos a entrenar. El test sirve para detectar overfitting: si el modelo está sobreajustado, su PF en la corrida real va a ser altísimo, pero también lo será en las permutaciones (porque vuelve a memorizar ruido), y por eso el p-value sale alto.
...
- **MCPT walk-forward (WF)**: en cada fold entrenamos con la ventana de los últimos 8 años y predecimos los próximos 3 (re-entrenando cada 756 días). Concatenamos todas las predicciones OOS y medimos PF. Bajo $H_0$ permutamos $y$ y los retornos en el rango OOS conjunto una sola vez por permutación, y volvemos a correr el walk-forward completo.

  Aquí Masters insiste en una regla: la primera ventana de training nunca aparece en OOS en la corrida real, así que tampoco debe permutarse hacia adentro del OOS de las permutaciones.

## ¿Qué es exactamente un *sliding walk-forward*?

El walk-forward es la forma honesta de evaluar un modelo en series de tiempo. En *machine learning* tradicional uno parte los datos al azar en train/test, pero en finanzas eso no funciona: si entrenamos con 2020 y testeamos con 2010, estaríamos usando el futuro para predecir el pasado (*look-ahead bias*) y cualquier modelo se vería artificialmente bueno.

El walk-forward respeta el orden temporal. Hay dos variantes:

- **Anclado (*anchored*)**: la ventana de training crece con el tiempo. Siempre arranca en el primer dato y se extiende hasta el momento actual. El modelo "recuerda" toda la historia.
- **Deslizante (*sliding* o *rolling*)**: la ventana de training tiene tamaño fijo y se desliza hacia adelante. El modelo "olvida" datos viejos y siempre se entrena sobre las últimas N observaciones.

Usamos sliding walk-forward. El mercado tiene memoria limitada: los regímenes, las volatilidades y las correlaciones cambian con el tiempo, así que entrenar sobre una ventana móvil hace que el modelo se adapte a las condiciones recientes en lugar de arrastrar regímenes obsoletos.
...
En nuestro experimento sobre SPY, con ventana de training de 8 años y ventana OOS de 3 años sobre ~26 años de datos, salen exactamente 6 folds OOS (5 folds completos de 3 años más uno final de ~2.5 años, dado que el rango OOS total es ~17.5 años). Cada fold re-entrena el modelo desde cero sobre su ventana; no hay *online learning* ni transferencia entre folds. Usamos folds OOS largos (3 años) en lugar de cortos (3 meses) porque el test queda más exigente: el modelo tiene que generalizar lejos de su training set, y se parece más a la práctica realista de un sistema que se re-entrena cada cierto tiempo en lugar de cada trimestre.
...
1. **El train inicial queda anclado.** En la corrida real, esos primeros datos sólo aparecen como *training* del primer fold y no como ventana OOS en ningún fold. Si los permutáramos, podrían terminar metidos dentro de una ventana OOS bajo $H_0$, inyectando observaciones (un crash, una racha de alta volatilidad) que no estaban en la corrida real, y la comparación dejaría de ser apples-to-apples.

2. **Todo el rango OOS se permuta como un único bloque.** Permutar cada fold por separado es la alternativa obvia, pero destruye la estructura temporal de largo plazo (tendencia, *volatility clustering*, autocorrelación) del rango OOS y trata a cada fold como un universo independiente. Permutar el bloque completo preserva esa estructura.
...
Lo importante no es el modelo ni cuántos folds salen, sino qué se permuta y qué no. El train inicial queda intacto (zona gris) y el rango OOS se baraja como un solo bloque (zona roja). Esa disciplina es la que hace al test honesto.
...
Aquí dedicamos una sección entera a explicar con detalle cómo permutamos en cada test. Es la decisión metodológica más delicada del trabajo: si esto se hace mal, todo lo demás vale poco.
...
La regla de oro es que $y$ y $r$ se deben permutar con el mismo índice. La razón es que $y_t = \text{sign}(r_{t+1})$: si los barajamos por separado, esa identidad se rompe y aparecen targets imposibles dado los retornos. Mantener el par junto preserva la coherencia interna del "mundo permutado".
...
Aquí la cosa es más fina. Masters lo dice claro: en walk-forward la primera ventana de training nunca aparece en OOS en la corrida real, así que no debe permutarse hacia adentro del rango OOS de las permutaciones; si lo hacemos, podríamos inyectar datos atípicos (un crash, un periodo de alta volatilidad) que distorsionan la comparación.

Nuestra estrategia (la opción de Masters, "do a single permutation of all market changes after the first training fold"):

1. Pre-OOS (primeras 2,016 barras, ~2000-01-03 a 2008-01-04): no se permuta. Es el train inicial. La curva de equity es idéntica en todas las permutaciones en esa zona.
...
Dentro de cada fold walk-forward, el modelo se entrena con la `y_used` (eventualmente permutada) y se evalúa contra `ret_used` (permutado de forma consistente). Eso rompe el link $X \to (y, r)$ sólo dentro del rango OOS. La parte pre-OOS sirve de anclaje común a todas las trayectorias.
...
Este es el plot que más nos sorprendió al armar el experimento. El LGBM produce un PF de 16.9 sobre los datos reales, y un PF promedio de 14 sobre $y$ permutadas al azar. Es Masters en acción:
...
El modelo es tan flexible que vuelve a memorizar el ruido cada vez que le pasamos un $y$ permutado distinto. El backtest "espectacular" (PF = 16.9) es prácticamente indistinguible de lo que se obtiene con un target aleatorio. Si alguien sólo viera el PF real, concluiría que tiene un sistema genial; el MCPT muestra que casi todo ese PF es overfitting.

El p-value de 0.044 es engañosamente "significativo". La razón por la que no salió cercano a 1 la discutimos más abajo: hay features correlacionadas con la magnitud de $r_{t+1}$ que le dan una ventaja sutil a la corrida real.
...
Aquí restringimos las features a OHLC crudos para minimizar la correlación con $|r_{t+1}|$. El PF real (1.88) sigue siendo más alto que el promedio bajo $H_0$ (1.75), pero la diferencia absoluta es chica. El modelo encuentra "algo", pero el efecto es modesto.

La diferencia entre LGBM (PF $\approx 14$ bajo $H_0$) y LGBM clean (PF $\approx 1.75$ bajo $H_0$) cuantifica el sesgo que meten las features correlacionadas con la magnitud del retorno. Sin esas features, el test se comporta mucho más limpiamente.
...
El mensaje pedagógico que más nos importa: una curva de equity que sube limpiamente de 0 a 45 a lo largo de 26 años de "backtest" puede no significar absolutamente nada. Cualquier $y$ permutada al azar produce algo equivalente cuando el modelo es lo suficientemente flexible.
...
En estas gráficas la línea vertical punteada marca el inicio del rango OOS (fin de la primera ventana de training, $\approx$ 2008). Hasta ahí todas las trayectorias son idénticas porque no se permuta; a partir de ese punto el abanico se abre.
...
![WF — LightGBM: hasta 2008 la curva (entrenamiento in-sample) sube de 0 a 17. A partir de 2008 (zona OOS), la curva real se vuelve plana y queda completamente dentro del abanico. Toda la "ganancia" pre-2008 era in-sample.](results/output13.png){#fig-eq-wf-lgbm}

Probablemente es la imagen más didáctica del reporte. La curva pre-OOS muestra cómo se vería un "backtest" tradicional sin walk-forward: PnL subiendo limpio de 0 a 17 a lo largo de 8 años. Cualquier presentación vendiendo una estrategia con esa curva queda convincente. Pero apenas el modelo tiene que predecir el futuro, no hay diferencia con permutaciones aleatorias.
...
Fuente del sesgo: features como `rvol_5d`, `rvol_21d` y `atr_norm_14d` correlacionan con $|r_{t+1}|$, es decir, saben cuándo el mercado está volátil aunque no en qué dirección. Bajo Y-permutation, al barajar $(y, r)$, los retornos grandes se asocian con días donde la volatilidad reciente podía ser cualquiera. Pero en la corrida real los retornos grandes coinciden con días donde la volatilidad reciente era genuinamente alta, y el modelo, sin saber el signo, igual gana porque concentra sus posiciones (en cualquier dirección) en los días de retorno absoluto grande.

Eso rompe el supuesto de Masters de "*equal probability under $H_0$*": las permutaciones no son intercambiables con la realidad porque la correlación entre features y $|r|$ es asimétrica.
...
1. El MCPT discrimina bien entre modelos con señal y sin señal. El oráculo obtiene p-values $< 0.01$ tanto IS como WF, y el random p-values dispersos en el rango medio, consistentes con $H_0$.

2. El MCPT in-sample detecta overfitting. LightGBM produce PF in-sample de 16.9 (espectacular en un backtest tradicional), pero el promedio bajo permutación es 14: el modelo aprende ruido sobre cualquier $y$ que se le presente. El WF lo confirma: el mismo modelo cae a PF = 1.06 OOS, dentro de la distribución nula. Sin permutation testing, todo esto sería invisible.

3. Las features correlacionadas con la magnitud del retorno contaminan el Y-permutation test. Comparar LGBM (PF nulo $\approx 14$) contra LGBM clean (PF nulo $\approx 1.75$) pone número al sesgo. Implicación práctica: la elección de features no sólo es un tema de poder predictivo, también lo es de correlación con $|r|$ si no queremos romper la validez del MCPT.

4. Walk-forward es el filtro definitivo. La caída PF 16.9 $\to$ 1.06 entre IS y WF es la firma cuantitativa del sobreajuste.
...
- Una sola realización del control negativo (random). Para verificar formalmente la uniformidad del p-value bajo $H_0$ habría que repetir el experimento con varios `REAL_SEED`. Es caro computacionalmente.
- Y-permutation supone target sin autocorrelación. El signo del log-return diario tiene autocorrelación cercana a cero, pero no exactamente cero, y en *timeframes* intradía el supuesto se puede romper.
...
El notebook con todo el código ejecutable está en `mcpt_calibration.ipynb`. La simulación toma del orden de 2-3 horas en una máquina con 8 cores. Para reproducir los resultados:
